      ...
      <div className="mx-auto max-w-7xl">
        ...
        <section className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,1.35fr)_minmax(320px,0.65fr)]">
          <div className="rounded-[30px] bg-[#123f38] px-6 py-8 text-white shadow-[0_24px_70px_rgba(18,63,56,0.18)] sm:px-9">
            ...
            <h1 className="mt-3 max-w-3xl font-serif text-4xl sm:text-5xl">
              {hasPets ? "Retrouvez ici les documents de vos animaux." : "Ajoutez votre premier animal."}
            </h1>
            <p className="mt-4 max-w-2xl text-sm leading-7 text-[#c4d7d2]">
              {hasPets
                ? "Ouvrez une fiche pour consulter les rapports, le poids et les rappels."
            ...
            <div className="mt-7 flex flex-wrap gap-3">
              ...
              {!hasPets && (
                <Link href="/analyser" className="rounded-full border border-white/20 bg-white/10 px-5 py-3 text-sm font-semibold text-white hover:bg-white/15">
                  Ajouter d’abord un document
                </Link>
              )}
              {hasPets && (
            ...
            {subscriptionActive ? (
              <>
                <p className="mt-4 text-sm leading-6 text-[#526f68]">
                  Vous recevez un crédit chaque mois. Les crédits non utilisés restent disponibles, jusqu’à trois, pour n’importe lequel de vos animaux.
                </p>
        ...
        <section className="mt-8">
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div>
              ...
              <h2 className="mt-2 font-serif text-3xl text-[#123f38]">Leurs fiches</h2>
            </div>
                  ...
                  <Link key={pet.id} href={`/dashboard/animaux/${pet.id}`} className="group rounded-[26px] border border-[#dce7e2] bg-white p-6 shadow-[0_14px_40px_rgba(31,78,67,0.06)] transition hover:-translate-y-1 hover:border-[#9fc9bc] hover:shadow-[0_20px_55px_rgba(31,78,67,0.12)]">
                    ...
                    <div className="mt-5 grid grid-cols-2 gap-2">
                      ...
                      <div className="rounded-xl bg-[#f5f8f7] px-3 py-2.5">
                        ...
                        <p className="mt-1 truncate text-sm font-semibold text-[#315f57]">{latest ? STATUS_LABELS[latest.status] ?? latest.status : "Aucun document"}</p>
                      </div>
                    </div>
                  </Link>
                );
        ...
        <div className="mt-8 grid gap-7 lg:grid-cols-2">
          <section className="rounded-[26px] border border-[#dce7e2] bg-white p-6 shadow-[0_14px_40px_rgba(31,78,67,0.06)]">
            ...
            <h2 className="mt-2 font-serif text-2xl text-[#123f38]">Vos derniers documents</h2>
              ...
              {(cases ?? []).slice(0, 5).map((item) => {
                ...
                return (
                  <Link key={item.id} href={`/dashboard/dossiers/${item.id}`} className="flex items-center justify-between gap-4 rounded-2xl border border-[#e0e9e6] px-4 py-3 hover:border-[#a9cec2]">
                    <div>
                      <p className="text-sm font-semibold text-[#315f57]">{petName || "Animal"} · {item.document_type === "facture" ? "Facture" : "Devis"}</p>
                      <p className="mt-1 text-xs text-[#78908a]">{formatDate(item.created_at)}</p>
                    </div>
                    <span className="rounded-full bg-[#edf6f2] px-3 py-1.5 text-[11px] font-semibold text-[#397268]">{STATUS_LABELS[item.status] ?? item.status}</span>
                  </Link>
                );
              })}
